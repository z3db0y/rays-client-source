        ...
        for(var category of categories) {
            ...
            if(settSearch.length > 0) {
                ...
                for(var sett of settSearch) {
                    ...
                    switch(sett.type) {
                        case 'toggle':
                            settHTML += `<label class="switch" style="margin-left:10px"><input type="checkbox" id="${sett.id.replace(/"/g, '\\"')}" ${config.get(sett.id, false) ? 'checked' : ''} onclick="setClientSetting(this.id, this.checked)"><span class="slider"><span class="grooves"></span></span></label></div>`;
                            break;
                        case 'input':
                            ...
                            break;
                        case 'select':
                            settHTML += `<select id="${sett.id.replace(/"/g, '\\"')}" onchange="setClientSetting(this.id, this.value)" class="inputGrey2" value="${config.get(sett.id, '').replace(/"/g, '\\"')}">`;
                            for(var option of sett.options) {
                                settHTML += `<option value="${option.value}">${option.name}</option>`;
                            }
                            settHTML += `</select></div>`;
                            break;
                        case 'button':
                            ...
                            break;
                        case 'slider':
                            settHTML += `<div class="slidecontainer" style="margin-left: auto"><input type="range" id="${sett.id.replace(/"/g, '\\"')}_slider" min="${sett.min}" max="${sett.max}" step="${sett.step}" value="${config.get(sett.id, 1)}" class="sliderM" oninput="setClientSetting('${sett.id.replace(/'/g, '\\\'')}', this.value), (document.getElementById('${sett.id.replace(/'/g, '\\\'')}').value = this.value)"></div><input type="number" class="sliderVal" id="${sett.id.replace(/"/g, '\\"')}" min="${sett.min}" max="${sett.max}" value="${config.get(sett.id, 1)}" oninput="!isNaN(parseFloat(this.value)) && (document.getElementById('${sett.id.replace(/'/g, '\\\'')}_slider').value = this.value) && setClientSetting(this.id, this.value)" style="margin-right:0px;margin-left:0 !important;border-width:0px"></div>`;
                            break;
                        case 'custom':
